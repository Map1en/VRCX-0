...
VRCX-0 是 VRCX 的完全重寫版本，由 VRCX 前任維護者之一開發，後端為 Rust（Tauri + React），效能大幅提升：多年累積的歷史資料也不會卡，記憶體和安裝體積都遠小於原版。

首次啟動會自動匯入你現有的 VRCX 資料與設定，原始資料不會被更動，隨時可以換回去。

VRCX 已是成熟穩定的版本，VRCX-0 則持續開發新功能。
...
- **背景模式**下記憶體進一步降至僅數十 MB，所有核心功能照常運作
- **比一個模型還小** — 安裝程式 10 多 MB，安裝後 30 多 MB，比 VRCX 小 10 倍以上
- **遷移零負擔** — 自動匯入 VRCX 的資料庫與設定，原始資料不會被更動

### VRCX-0 獨有

- **AI 助手** — 幫你回顧自己的 VRChat 生活：最常和誰一起玩、正在和誰漸行漸遠、什麼時候上線最容易遇到好友，接入你自己的 AI 服務即可使用
- **側欄模式** — 忙別的事時，用一條窄側欄留意好友動態，不必開著大視窗來回切換；Windows 和 macOS 可停靠在螢幕邊緣並自動隱藏
- **快捷鍵與全域快捷鍵** — 切換頁面、開啟設定、切換分頁都不必碰滑鼠；Windows 可設定全域快捷鍵，在其他應用程式中隨時叫出或隱藏 VRCX-0
- **分享世界收藏集** — 把收藏的世界做成可分享的頁面，對方可以瀏覽、開啟或匯入；也支援單獨分享世界和角色連結
- **MCP 伺服器** — 讓外部 AI 工具直接存取你的本機社交資料，比 AI 助手靈活得多；適合進階使用者
- **整合 API** — 面向第三方應用程式，提供遊戲中的房間即時資料，後續逐步擴充
...
| 功能           | VRCX                                                                                               | VRCX-0（+ 為新增）                                                                                                         |
| -------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| **社交自動化** | 獨處或有人在場時切換狀態與簽名（可限定房間類型）；自動回覆邀請請求                                 | + 定時規則（時段、星期）、多條情境規則（在場好友、人數、房間類型、收藏世界，**可設優先順序**）、**規則結束時還原原本狀態** |
| **通知**       | 桌面、語音播報、XSOverlay、OVR Toolkit、腕部懸浮層（各通道支援的事件不同，只有懸浮層能依事件篩選） | + **Discord 格式 Webhook**、勿擾模式；所有通道支援相同事件，各自依事件獨立設定                                             |
| **VR 懸浮層**  | 腕部與頭顯，瀏覽器算繪（記憶體佔用 100 MB+）；OpenVR                                               | + OpenXR（**WiVRn 實測通過**）；原生算繪（記憶體佔用數十 MB）                                                              |
| 螢幕截圖       | 檢視與搜尋中繼資料                                                                                 | + 格狀檢視、多選、批次刪除、匯出 ZIP                                                                                       |
| 角色資訊       | 效能等級與檔案大小                                                                                 | + 三角面數、貼圖記憶體、材質、骨頭、PhysBone、粒子、碰撞體（對照各平台上限）                                               |
| 備份           | VRChat 登錄檔設定                                                                                  | + 資料庫定期備份、多版本、一鍵還原                                                                                         |
| 群組管理       | 在個人資料內調整順序，逐一設定可見性                                                               | + 「我的群組」頁面（批次退出、批次設定可見性）                                                                             |
| 主題           | 內建主題、自訂 CSS（需手動放置檔案）                                                               | + 社群主題目錄、背景圖片、應用程式內 CSS 編輯器、強調色                                                                    |
