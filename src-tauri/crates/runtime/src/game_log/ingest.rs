use vrcx_0_domain::log_watcher::{GameLogEvent, GameLogEventKind};
...
impl GameLogIngestEngine {
    ...
    pub fn ingest_events(
        ...
        for event in events {
            ...
            if batch_write_count(&output.batch) > write_count_before {
                output
                    .backend_persisted_mirrors
                    .push(event.to_compat_row());
            }
        }
    ...
    fn ingest_resource_load(
        ...
        if resource_url.is_empty() || self.state.last_resource_url == resource_url || !log_resource_load
        {
            return;
        }
...
mod tests {
    ...
    #[test]
    fn resource_load_without_write_does_not_emit_backend_persisted_mirror() {
        ...
        assert!(output.backend_persisted_mirrors.is_empty());
    }
    ...
    fn external_vrcx_event_emits_mirror_when_external_row_is_written() {
        ...
        assert_eq!(output.backend_persisted_mirrors.len(), 1);
        assert_eq!(output.backend_persisted_mirrors[0][2], "vrcx");
    }
}
