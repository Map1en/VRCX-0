...
fn update(
    ...
    let till_date_utc = inner.till_date.lock().unwrap().unwrap_or(chrono::DateTime::UNIX_EPOCH.naive_utc());
    ...
    for entry in entries {
        ...
        parse_log(inner, app_handle, &entry.path(), &name, ctx, till_date, *first_run);
    }
...
fn parse_log(
    ...
    loop {
        ...
        if content.starts_with('[') {
            ...
            let _ = parse_player_joined_or_left(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_location(inner, app_handle, file_name, trimmed, content, ctx, first_run)
                || parse_location_destination(inner, app_handle, file_name, trimmed, content, ctx, first_run)
                ...
                || parse_avatar_pedestal_change(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_video_error(inner, app_handle, file_name, trimmed, content, ctx, first_run)
                || parse_video_change(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_avpro_video_change(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_usharp_video_play(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_usharp_video_sync(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_world_vrcx(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_audio_config(inner, app_handle, file_name, trimmed, content, ctx, first_run)
                ...
                || parse_vote_kick_success(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_sticker_spawn(inner, app_handle, file_name, trimmed, content, first_run);
        } else {
            
            let _ = parse_shader_keywords_limit(inner, app_handle, file_name, trimmed, content, ctx, first_run)
                || parse_sdk2_video_play(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_application_quit(inner, app_handle, file_name, trimmed, content, ctx, first_run)
                || parse_openvr_init(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_desktop_mode(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_osc_failed(inner, app_handle, file_name, trimmed, content, first_run)
                || parse_untrusted_url(inner, app_handle, file_name, trimmed, content, ctx, first_run);
        }
    }
    ...
    match NaiveDateTime::parse_from_str(&line[..19], "%Y.%m.%d %H:%M:%S") {
        Ok(local_dt) => {
            ...
            match local_aware.single() {
                Some(dt) => dt.with_timezone(&Utc).format("%Y-%m-%dT%H:%M:%S%.3fZ").to_string(),
...
fn parse_location(
    inner: &Inner, app: &AppHandle, fname: &str, line: &str, content: &str,
    ctx: &mut LogContext, first_run: bool,
) -> bool {
        ...
        && !content.contains("] Joining friend: ")
    {
        if let Some(pos) = line.rfind("] Joining ") {
            ...
            append_log(inner, app, vec![
                fname.into(), convert_log_time_to_iso8601(line),
                "location".into(), location, ctx.recent_world_name.clone(),
            ], first_run);
...
fn parse_location_destination(
    ...
    if content.contains("[Behaviour] OnLeftRoom") {
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line),
            "location-destination".into(), ctx.location_destination.clone(),
        ], first_run);
...
fn parse_player_joined_or_left(
    inner: &Inner, app: &AppHandle, fname: &str, line: &str, content: &str, first_run: bool,
) -> bool {
    if content.contains("[Behaviour] OnPlayerJoined") && !content.contains("] OnPlayerJoined:") {
        if let Some(pos) = line.rfind("] OnPlayerJoined") {
            ...
            if !display_name.is_empty() || !user_id.is_empty() {
                append_log(inner, app, vec![
                    fname.into(), convert_log_time_to_iso8601(line),
                    "player-joined".into(), display_name, user_id,
                ], first_run);
            }
        }
        return true;
        ...
        && !content.contains("] OnPlayerLeft:")
    {
        if let Some(pos) = line.rfind("] OnPlayerLeft") {
            ...
            if !display_name.is_empty() || !user_id.is_empty() {
                append_log(inner, app, vec![
                    fname.into(), convert_log_time_to_iso8601(line),
                    "player-left".into(), display_name, user_id,
                ], first_run);
            }
        }
        return true;
...
fn parse_portal_spawn(
    inner: &Inner, app: &AppHandle, fname: &str, line: &str, first_run: bool,
) -> bool {
    if line.contains("[Behaviour] Instantiated a (Clone [") && line.contains("] Portals/PortalInternalDynamic)") {
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line), "portal-spawn".into(),
        ], first_run);
        return true;
    }
    false
...
fn parse_avatar_change(
    ...
    if !content.starts_with("[Behaviour] Switching ") { return false; }
    if let Some(pos) = line.rfind(" to avatar ") {
        if let Some(start) = line.rfind("[Behaviour] Switching ") {
            ...
            append_log(inner, app, vec![
                fname.into(), convert_log_time_to_iso8601(line),
                "avatar-change".into(), display_name.into(), avatar_name.into(),
            ], first_run);
        }
    }
    true
...
fn parse_join_blocked(
    ...
    if !content.contains("] Master is not sending any events! Moving to a new instance.") { return false; }
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line),
        "event".into(), "Joining instance blocked by master".into(),
    ], first_run);
    true
}
...
fn parse_avatar_pedestal_change(
    ...
    if !content.starts_with(tag) { return false; }
    let data = &content[tag.len()..];
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line),
        "event".into(), format!("{data} changed avatar pedestal"),
    ], first_run);
    true
}
...
fn parse_video_error(
    ...
    if content.contains("[Video Playback] ERROR: ") {
        if let Some(pos) = content.find("[Video Playback] ERROR: ") {
            ...
            if !ctx.video_errors.insert(data.clone()) { return true; }
            if data.contains(YT_BOT_ERROR) { data = format!("{YT_BOT_FIX}\n{data}"); }
            append_log(inner, app, vec![
                fname.into(), convert_log_time_to_iso8601(line),
                "event".into(), format!("VideoError: {data}"),
            ], first_run);
        }
        return true;
    }
...
fn parse_world_vrcx(
    ...
    if !content.starts_with("[VRCX] ") { return false; }
    let data = &content[7..];
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line),
        "vrcx".into(), data.into(),
...
fn parse_screenshot(
    ...
    if !content.contains("[VRC Camera] Took screenshot to: ") { return false; }
    if let Some(pos) = line.rfind("] Took screenshot to: ") {
        let path = &line[pos + 22..];
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line),
            "screenshot".into(), path.into(),
        ], first_run);
    }
    true
}
...
fn parse_shader_keywords_limit(
    ...
    if !content.contains("Maximum number (384) of shader global keywords exceeded") { return false; }
    if ctx.shader_keywords_limit_reached { return true; }
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line),
        "event".into(), "Shader Keyword Limit has been reached".into(),
    ], first_run);
...
fn parse_openvr_init(
    ...
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line), "openvr-init".into(),
    ], first_run);
    true
}
...
fn parse_desktop_mode(
    ...
    if !content.starts_with("VR Disabled") { return false; }
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line), "desktop-mode".into(),
    ], first_run);
    true
}
...
fn parse_string_download(
    ...
    if !content.contains(tag) { return false; }
    if let Some(pos) = line.rfind(tag) {
        let rest = &line[pos + tag.len()..];
        if let Some(end) = rest.rfind('\'') {
            let url = &rest[..end];
            if url.starts_with("http://127.0.0.1:22500") || url.starts_with("http://localhost:22500") {
                return true;
            }
            append_log(inner, app, vec![
                fname.into(), convert_log_time_to_iso8601(line),
                "resource-load-string".into(), url.into(),
            ], first_run);
        }
    }
    true
...
fn parse_image_download(
    ...
    if let Some(pos) = line.rfind(tag) {
        ...
        if let Some(end) = rest.rfind('\'') {
            ...
            append_log(inner, app, vec![
                fname.into(), convert_log_time_to_iso8601(line),
                "resource-load-image".into(), url.into(),
            ], first_run);
        }
    }
    true
...
fn parse_vote_kick(
    ...
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line),
        "event".into(), content[tag.len()..].into(),
    ], first_run);
    true
}
...
fn parse_osc_failed(
    ...
    if !content.starts_with("Could not Start OSC: ") { return false; }
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line),
        "event".into(), format!("VRChat couldn't start OSC server, \"{content}\""),
    ], first_run);
    true
}
...
fn parse_untrusted_url(
    ...
    if !content.contains("Attempted to play an untrusted URL") { return false; }
    if !ctx.video_errors.insert(content.to_string()) { return true; }
    append_log(inner, app, vec![
        fname.into(), convert_log_time_to_iso8601(line),
        "event".into(), format!("VideoError: {content}"),
    ], first_run);
    true
}
...
fn parse_instance_reset(
    ...
    if !content.contains("[ModerationManager] This instance will be reset in ") { return false; }
    if let Some(pos) = content.find("[ModerationManager] ") {
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line),
            "event".into(), content[pos + 20..].into(),
        ], first_run);
    }
    true
}
...
fn parse_vote_kick_init(
    ...
    if !content.contains("[ModerationManager] A vote kick has been initiated against ") { return false; }
    if let Some(pos) = content.find("[ModerationManager] ") {
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line),
            "event".into(), content[pos + 20..].into(),
...
fn parse_vote_kick_success(
    ...
    if !content.contains("[ModerationManager] Vote to kick ") { return false; }
    if let Some(pos) = content.find("[ModerationManager] ") {
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line),
            "event".into(), content[pos + 20..].into(),
...
fn parse_sticker_spawn(
    ...
    if let Some(pos) = content.find("[StickersManager] User ") {
        ...
        if display_name.is_empty() && user_id.is_empty() { return true; }
        ...
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line),
            "sticker-spawn".into(), user_id, display_name, inv_id,
        ], first_run);
    }
    true
}
...
fn parse_audio_config(
    ...
    if content.contains("[Always] uSpeak: SetInputDevice 0") {
        if let Some(pos) = line.rfind(") '") {
            ...
            append_log(inner, app, vec![
                fname.into(), convert_log_time_to_iso8601(line),
                "event".into(), format!("Audio device changed, mic set to '{audio_device}'"),
            ], first_run);
            ctx.last_audio_device = audio_device.to_string();
            ctx.audio_device_changed = false;
        }
...
fn parse_udon_exception(
    ...
    if let Some(pos) = line.find(" ---> VRC.Udon.VM.UdonVMException: ") {
        append_log(inner, app, vec![
            fname.into(), convert_log_time_to_iso8601(line),
            "udon-exception".into(), line[pos..].into(),
        ], first_run);
        return true;
    }
    false
