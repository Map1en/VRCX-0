use std::sync::Arc;

use base64::{engine::general_purpose::STANDARD as B64, Engine};
use futures_util::{SinkExt, StreamExt};
use tokio::sync::Mutex;
use tokio_tungstenite::tungstenite::Message;

/// OVRToolkit WebSocket notification client.
///
/// Port of C# OVRToolkit.cs — lazy-connects to `ws://127.0.0.1:11450/api`
/// and keeps a persistent connection, reconnecting on failure.
pub struct OvrToolkit {
    sender: Arc<Mutex<Option<WsSender>>>,
}

type WsSender = futures_util::stream::SplitSink<
    tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>,
    Message,
>;

// Hard-coded VRCX icon as base64 (same blob as C#).
// C# System.Text.Json serializes byte[] as base64 — OVRToolkit API expects this format.
const VRCX_ICON_B64: &str = "";

impl OvrToolkit {
    pub fn new() -> Self {
        Self {
            sender: Arc::new(Mutex::new(None)),
        }
    }

    /// Send an OVRToolkit notification.
    ///
    /// `timeout` and `opacity` are accepted for API compatibility but currently
    /// unused by OVRToolkit (same as C# — marked `[CURRENTLY UNUSED]` there).
    pub fn send_notification(
        &self,
        hud_notification: bool,
        wrist_notification: bool,
        title: &str,
        body: &str,
        _timeout: i32,
        _opacity: f64,
        image: Option<&str>,
    ) {
        let mut messages: Vec<serde_json::Value> = Vec::new();

        // Read custom icon and base64-encode, or fall back to embedded base64 icon.
        // OVRToolkit API expects icon as a base64 string (matching C# byte[] JSON serialization).
        let icon_b64: String = image
            .filter(|p| !p.is_empty() && std::path::Path::new(p).exists())
            .and_then(|p| std::fs::read(p).ok())
            .map(|bytes| B64.encode(&bytes))
            .unwrap_or_else(|| VRCX_ICON_B64.to_string());

        if wrist_notification {
            messages.push(serde_json::json!({
                "messageType": "SendWristNotification",
                "json": serde_json::to_string(&serde_json::json!({
                    "body": format!("{title} - {body}")
                })).unwrap_or_default()
            }));
        }

        if hud_notification {
            messages.push(serde_json::json!({
                "messageType": "SendNotification",
                "json": serde_json::to_string(&serde_json::json!({
                    "title": title,
                    "body": body,
                    "icon": icon_b64
                })).unwrap_or_default()
            }));
        }

        if messages.is_empty() {
            return;
        }

        let sender = self.sender.clone();
        tokio::spawn(async move {
            if let Err(e) = send_with_persistent_conn(sender, messages).await {
                tracing::error!("[OVRToolkit] send error: {e}");
            }
        });
    }
}

/// Open a fresh WebSocket connection and spawn a background reader to drain
/// incoming frames (keeps the connection alive for ping/pong handling).
async fn connect_ws() -> Result<WsSender, String> {
    let url = "ws://127.0.0.1:11450/api";
    let (ws_stream, _) = tokio_tungstenite::connect_async(url)
        .await
        .map_err(|e| format!("connect: {e}"))?;

    let (write, read) = ws_stream.split();

    // Drain incoming frames in background so pings are answered and the
    // connection is not considered dead by the server.
    tokio::spawn(async move {
        let mut read = read;
        while read.next().await.is_some() {}
    });

    Ok(write)
}

/// Send all messages on `ws`, returning an error if any write fails.
async fn send_all(ws: &mut WsSender, messages: &[serde_json::Value]) -> Result<(), String> {
    for msg in messages {
        let text = serde_json::to_string(msg).unwrap_or_default();
        ws.send(Message::Text(text.into()))
            .await
            .map_err(|e| format!("send: {e}"))?;
    }
    Ok(())
}

/// Try sending on the existing persistent connection; on failure, reconnect
/// once and retry.  The connection is stored for future calls.
async fn send_with_persistent_conn(
    sender: Arc<Mutex<Option<WsSender>>>,
    messages: Vec<serde_json::Value>,
) -> Result<(), String> {
    let mut guard = sender.lock().await;

    // Attempt to reuse an existing connection.
    if let Some(ref mut ws) = *guard {
        if send_all(ws, &messages).await.is_ok() {
            return Ok(());
        }
        // Connection broken — drop and reconnect below.
        *guard = None;
    }

    // Fresh connection.
    let mut ws = connect_ws().await?;
    send_all(&mut ws, &messages).await?;
    *guard = Some(ws);
    Ok(())
}