...
impl WebClient {
    pub fn new(proxy_url: Option<String>, cookies_b64: Option<&str>) -> Result<Self> {
        ...
        if let Some(cookies_b64) = cookies_b64 {
            wc.restore_cookies(cookies_b64);
        }
    ...
    pub async fn execute(&self, options: HashMap<String, Value>) -> Result<(i32, String)> {
        let url = options
            .get("url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Custom("Missing request URL".into()))?
            .to_string();
    ...
    fn build_file_put_request(
        ...
        let file_data = options
            .get("fileData")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Custom("missing fileData".into()))?;
    ...
    fn build_legacy_image_upload_request(
        ...
        let image_data = options
            .get("imageData")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Error::Custom("missing imageData".into()))?;
...
mod tests {
    ...
    fn execute_returns_success_status_and_body_for_daily_get() -> Result<()> {
        ...
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        let result = runtime.block_on(web.execute(options));
        server.join().unwrap();
