...
use vrcx_0_application::profile::{
    ...
    DataDirMigrationMode, DataDirMigrationPhase, DataDirMigrationPlan, DataDirMigrationPort,
    DataDirMigrationState, DataDirMigrationStatus, DataDirPointerCommitter, OperationGuard,
...
#[derive(Clone)]
pub struct LocalDataDirMigrationPort {
...
impl LocalDataDirMigrationPort {
    pub fn new(
        source_dir: PathBuf,
        control_dir: PathBuf,
        db: Arc<DatabaseService>,
...
impl DataDirMigrationPort for LocalDataDirMigrationPort {
    fn current_status(&self) -> DataDirMigrationStatus {
        LocalDataDirMigrationPort::current_status(self)
    }
    ...
    fn request_migration(
        ...
        LocalDataDirMigrationPort::request_migration(self, plan, mode)
    }
    ...
    fn run_migration(
        ...
        LocalDataDirMigrationPort::run_migration(self, target_dir, replace_existing)
    }

    fn request_cancel(&self) -> DataDirMigrationActionOutcome {
        LocalDataDirMigrationPort::request_cancel(self)
    }

    fn switch_data_dir_pointer(&self, target_dir: PathBuf) -> DataDirMigrationActionOutcome {
        LocalDataDirMigrationPort::switch_data_dir_pointer(self, target_dir)
    }

    fn take_last_result(&self) -> Result<Option<DataDirMigrationResult>> {
        LocalDataDirMigrationPort::take_last_result(self)
    }

    fn cleanup_pending(&self) -> Result<Option<DataDirCleanupPending>> {
        LocalDataDirMigrationPort::cleanup_pending(self)
    }

    fn cleanup_migrated_data(&self) -> Result<Option<DataDirCleanupReport>> {
        LocalDataDirMigrationPort::cleanup_migrated_data(self)
    }

    fn dismiss_cleanup(&self) -> Result<()> {
        LocalDataDirMigrationPort::dismiss_cleanup(self)
    }

    fn mark_cleanup_prompted(&self, prompted_at: String) -> Result<()> {
        LocalDataDirMigrationPort::mark_cleanup_prompted(self, prompted_at)
    }
}
