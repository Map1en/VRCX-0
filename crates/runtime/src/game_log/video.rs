...
pub async fn handle_video_play(
    ...
    event_bus.emit_backend_game_log_event(raw_row);
...
async fn lookup_youtube_video(
    ...
    let enabled = backend_config::get_bool(db, "youtubeAPI", false)?;
    let api_key = backend_config::get_string(db, "youtubeAPIKey", "")?;
    ...
    let (status, body) = web.execute(options).await?;
    if status != 200 {
        return Ok(None);
    }
