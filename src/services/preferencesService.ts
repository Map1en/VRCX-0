...
export async function getTablePageSizesPreference(
    fallback = DEFAULT_TABLE_PAGE_SIZES
) {
    const preferenceState = usePreferencesStore.getState();
    if (preferenceState.preferencesHydrated) {
...
export async function setTrustColorPreference(key: string, value) {
    if (
...
export async function setDiscordBoolPreference(key: string, value) {
