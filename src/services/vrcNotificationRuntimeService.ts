...
function normalizeV1Notification(data: Record<string, any>) {
...
function normalizeV2Notification(data: Record<string, any>, endpoint = '') {
    const ref = createDefaultNotificationV2Ref(
        sanitizeNotificationJson({ ...data })
    ) as any;
    ref.createdAt = ref.createdAt || ref.created_at || new Date().toISOString();
    ref.created_at = ref.createdAt;
    (ref as any).data = parseObject(ref.data);
    (ref as any).responses = parseArray(ref.responses);
...
function notifyNotificationMenu(notification: Record<string, any>) {
...
function shouldPersistV1(notification: Record<string, any>, currentUserId: string) {
    return (
...
async function persistV1Notification(notification: Record<string, any>) {
...
async function persistV2Notification(notification: Record<string, any>) {
    const currentUserId = await ensureNotificationTables();
    if (!currentUserId) {
        return;
    }
...
async function expireNotificationById(id: unknown) {
    const currentUserId = await ensureNotificationTables();
    if (!id || !currentUserId) {
        return;
    }
...
async function markV2NotificationSeen(id: unknown) {
    const currentUserId = await ensureNotificationTables();
    if (!id || !currentUserId) {
        return;
    }
...
async function runInviteAutomation(notification: Record<string, any>) {
...
export async function handleRealtimeNotificationEvent(
    type: string,
