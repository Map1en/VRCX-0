...
describe('notification table state helpers', () => {
    ...
    it('reads and writes persisted table state without dropping existing keys', () => {
        ...
        writePersistedNotificationTableState({ sorting: [{ id: 'type', desc: false }] });
    ...
    it('normalizes legacy column ids and keeps only sortable notification columns', () => {
        ...
        expect(sanitizeNotificationSorting([
        ...
        expect(sanitizeNotificationSorting([{ id: 'message', desc: true }])).toBe(
            NOTIFICATION_TABLE_DEFAULT_SORTING
        );
    });
    ...
    it('sanitizes filters, column visibility, order, sizing, and page size', () => {
        ...
        expect(sanitizeNotificationFilters(['invite', 'unknown', 'message'], allowedTypes)).toEqual([
            'invite',
            'message'
        ]);
        expect(sanitizeNotificationColumnVisibility({
            createdAt: false,
            sender: true,
            missing: false,
            message: 'yes'
        ...
        expect(sanitizeNotificationColumnOrder(['sender', 'senderUsername', 'message', 'missing'])).toEqual([
            'senderUsername',
            'message'
        ]);
        expect(sanitizeNotificationColumnSizing({
            createdAt: 120,
            message: '240',
            missing: 50,
            type: -1
        ...
        expect(resolveNotificationPageSize('bad')).toBe(NOTIFICATION_TABLE_DEFAULT_PAGE_SIZES[1]);
    });
});
