...
describe('tauri errors', () => {
    it('keeps Error instances when no extra fallback context is needed', () => {
        ...
        expect(normalizePlatformError(error, 'Backend command failed')).toBe(
            error
        );
    ...
    it('normalizes non-Error values into useful messages', () => {
        expect(
            normalizePlatformError(null, 'Backend command failed').message
        ).toBe('Backend command failed');
        expect(
            normalizePlatformError('denied', 'Backend command failed').message
        ).toBe('Backend command failed: denied');
        expect(
            normalizePlatformError({ code: 'E_FAIL' }, 'Backend command failed')
                .message
        ).toBe('Backend command failed: {"code":"E_FAIL"}');
    });
